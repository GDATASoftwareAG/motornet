    ...
    where TStartup : class
{
    ...
    public MotorTestHost<TStartup> ReplaceTransient<TInterface, TOldInstance, TNewInstance>()
        where TNewInstance : class =>
        Replace<TInterface, TOldInstance>(
            new ServiceDescriptor(typeof(TInterface), typeof(TNewInstance), ServiceLifetime.Transient)
        );

    public MotorTestHost<TStartup> ReplaceScoped<TInterface, TOldInstance, TNewInstance>()
        where TNewInstance : class =>
        Replace<TInterface, TOldInstance>(
            new ServiceDescriptor(typeof(TInterface), typeof(TNewInstance), ServiceLifetime.Scoped)
        );

    public MotorTestHost<TStartup> ReplaceSingleton<TInterface, TOldInstance, TNewInstance>()
        where TNewInstance : class =>
        Replace<TInterface, TOldInstance>(
            new ServiceDescriptor(typeof(TInterface), typeof(TNewInstance), ServiceLifetime.Singleton)
        );
    ...
    private MotorTestHost<TStartup> Replace<TInterface, TOldInstance>(ServiceDescriptor newServiceDescriptor)
    {
        ...
        {
            var oldServiceDescriptor = services.SingleOrDefault(s =>
                s.Lifetime == newServiceDescriptor.Lifetime
                && s.ServiceType == typeof(TInterface)
                && s.ImplementationType == typeof(TOldInstance)
            );
            ...
            else
            {
                throw new InvalidOperationException(
                    $"Could not find {newServiceDescriptor.Lifetime} service descriptor for {typeof(TInterface)} and {typeof(TOldInstance)}"
                );
            }
        });
        return this;
