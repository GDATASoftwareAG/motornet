...
public class CriticalFailureException : Exception
{
    ...
    where TData : notnull
{
    ...
    public async Task ExecuteAsync(CancellationToken token = default)
    {
        ...
        {
            throw new InvalidOperationException("PgMq consumer not initialized.");
        }
    ...
    private async Task ProcessNextMessageAsync(CancellationToken token)
    {
        ...
        {
            throw new UnhandledCloudEventFormatException(CloudEventFormat.Json);
        }
        ...
        try
        {
            status = await InvokeCallbackAsync(cloudEvent, token);
        }
        catch (OperationCanceledException) when (token.IsCancellationRequested)
        {
            _logger.LogWarning(
                LogEvents.TemporaryFailureOnConsume,
                "Message {MsgId} processing was cancelled, resetting visibility timeout so it is immediately redelivered after restart",
                message.MsgId
            );
            await _npgmqClient.SetVtAsync(_options.QueueName, message.MsgId, 0, CancellationToken.None);
            throw;
        }
        switch (status)
        {
    ...
    private MotorCloudEvent<byte[]> DecodeProtocolFormat(byte[] body, IReadOnlyDictionary<string, object> headers)
    {
        var cloudEvent = new MotorCloudEvent<byte[]>(
            ...
            body,
            new Uri($"pgmq://{_options.Host}:{_options.Port}/{_options.Database}?queue={_options.QueueName}")
        );
