...
public sealed class MetricsFactory<T> : IMetricsFactory<T>
{
    ...
    public IMetricFamily<IGauge, TLabels> CreateGauge<TLabels>(string name, string help, TLabels labelNames, bool includeTimestamp = false) where TLabels : struct, ITuple, IEquatable<TLabels>
    {
        var metricsName = PrependNamespace(name);
    ...
    public ISummary CreateSummary(string name, string help, bool includeTimestamp = false, IReadOnlyList<QuantileEpsilonPair>? objectives = null,
        TimeSpan? maxAge = null, int? ageBuckets = null, int? bufCap = null)
    {
        var metricsName = PrependNamespace(name);
