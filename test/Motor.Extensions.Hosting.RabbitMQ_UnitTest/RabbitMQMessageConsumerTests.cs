...
public class RabbitMQMessageConsumerTests
{
    ...
    public async Task StartAsync_CallbackConfigured_QueueDeclaredAndBind()
    {
        ...
        channelMock.Verify(
            x =>
                x.QueueBindAsync(
                    cfg.Queue.Name,
                    cfg.Queue.Bindings.First().Exchange,
                    cfg.Queue.Bindings.First().RoutingKey,
                    cfg.Queue.Bindings.First().Arguments,
                    false,
                    It.IsAny<CancellationToken>()
                ),
            Times.Exactly(1)
    ...
    public async Task StartAsync_ConsumerConfigWithHasTwoBindings_QueueIsBoundTwice()
    {
        ...
        channelMock.Verify(
            x =>
                x.QueueBindAsync(
                    cfg.Queue.Name,
                    cfg.Queue.Bindings.Last().Exchange,
                    cfg.Queue.Bindings.Last().RoutingKey,
                    cfg.Queue.Bindings.Last().Arguments,
                    false,
                    It.IsAny<CancellationToken>()
                ),
            Times.Exactly(1)
    ...
    )
    {
        ...
        return new RabbitMQMessageConsumer<string>(
            FakeLogger,
